import { WeatherCode, WeatherImage } from "../constants/constants";
...
function getWeatherImage(weatherCode: WeatherCode): string {
  const weather = items[weatherCode];
  if (weather) {
    return weather.day.image;
  }
  return 'https://openweathermap.org/img/wn/11n@2x.png'; // Return a default value or handle an unknown weather code
}
